...
modify(A) ::= WITH(B) iri(C) deleteClause(D) WHERE(E) groupGraphPattern(F). { A = new NTToken(); A->type = 541; A->copyBools(C); A->copyBools(D); A->copyBools(F); A->ssVars = F->ssVars + F->gGPssVars; A->vars = C->vars + D->vars + F->vars; A->bNodes = C->bNodes + D->bNodes + F->bNodes; A->query = 'WITH ' . C->query . PHP_EOL . D->query . PHP_EOL . 'WHERE' . PHP_EOL . F->query; A->childs = array(B, C, D, E, F); }
modify(A) ::= WITH(B) iri(C) insertClause(D) WHERE(E) groupGraphPattern(F). { A = new NTToken(); A->type = 541; A->copyBools(C); A->copyBools(D); A->copyBools(F); A->ssVars = F->ssVars + F->gGPssVars; A->vars = C->vars + D->vars + F->vars; A->bNodes = C->bNodes + D->bNodes + F->bNodes; A->query = 'WITH ' . C->query . PHP_EOL . D->query . PHP_EOL . 'WHERE' . PHP_EOL . F->query; A->childs = array(B, C, D, E, F); }
modify(A) ::= deleteClause(B) insertClause(C) usingClauseX(D) WHERE(E) groupGraphPattern(F). { A = new NTToken(); A->type = 541; A->copyBools(B); A->copyBools(C); A->copyBools(D); A->copyBools(F); A->ssVars = F->ssVars + F->gGPssVars; A->vars = B->vars + C->vars + D->vars + F->vars; A->bNodes = B->bNodes + C->bNodes + D->bNodes + F->bNodes; A->query = B->query . PHP_EOL . C->query . PHP_EOL . D->query . PHP_EOL . 'WHERE' . PHP_EOL . F->query; A->childs = array(B, C, D, E, F); }
modify(A) ::= deleteClause(B) usingClauseX(C) WHERE(D) groupGraphPattern(E). { A = new NTToken(); A->type = 541; A->copyBools(B); A->copyBools(C); A->copyBools(E); A->ssVars = E->ssVars + E->gGPssVars; A->vars = B->vars + C->vars + E->vars; A->bNodes = B->bNodes + C->bNodes + E->bNodes; A->query = B->query . PHP_EOL . C->query . PHP_EOL . 'WHERE' . PHP_EOL . E->query; A->childs = array(B, C, D, E); }
modify(A) ::= insertClause(B) usingClauseX(C) WHERE(D) groupGraphPattern(E). { A = new NTToken(); A->type = 541; A->copyBools(B); A->copyBools(C); A->copyBools(E); A->ssVars = E->ssVars + E->gGPssVars; A->vars = B->vars + C->vars + E->vars; A->bNodes = B->bNodes + C->bNodes + E->bNodes; A->query = B->query . PHP_EOL . C->query . PHP_EOL . 'WHERE' . PHP_EOL . E->query; A->childs = array(B, C, D, E); }
modify(A) ::= deleteClause(B) insertClause(C) WHERE(D) groupGraphPattern(E). { A = new NTToken(); A->type = 541; A->copyBools(B); A->copyBools(C); A->copyBools(E); A->ssVars = E->ssVars + E->gGPssVars; A->vars = B->vars + C->vars + E->vars; A->bNodes = B->bNodes + C->bNodes + E->bNodes; A->query = B->query . PHP_EOL . C->query . PHP_EOL . 'WHERE' . PHP_EOL . E->query; A->childs = array(B, C, D, E); }
...
graphPatternNotTriples(A) ::= groupOrUnionGraphPattern(B). { A = new NTToken(); A->type = 561; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->ssVars = B->ssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
graphPatternNotTriples(A) ::= optionalGraphPattern(B). { A = new NTToken(); A->type = 561; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->ssVars = B->ssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
graphPatternNotTriples(A) ::= minusGraphPattern(B). { A = new NTToken(); A->type = 561; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->ssVars = B->ssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
graphPatternNotTriples(A) ::= graphGraphPattern(B). { A = new NTToken(); A->type = 561; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->ssVars = B->ssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
graphPatternNotTriples(A) ::= serviceGraphPattern(B). { A = new NTToken(); A->type = 561; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->ssVars = B->ssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
...
optionalGraphPattern(A) ::= OPTIONAL(B) groupGraphPattern(C). { A = new NTToken(); A->type = 562; A->copyBools(C); A->gGPssVars = C->gGPssVars; A->vars = C->vars; A->bNodes = C->bNodes; A->query = 'OPTIONAL ' . C->query; A->childs = array(B, C); }

graphGraphPattern(A) ::= GRAPH(B) varOrIri(C) groupGraphPattern(D). { A = new NTToken(); A->type = 563; A->copyBools(D); A->gGPssVars = D->gGPssVars; A->vars = C->vars + D->vars; A->bNodes = D->bNodes; A->query = 'GRAPH ' . C->query . ' ' . D->query; A->childs = array(B, C, D); }
...
minusGraphPattern(A) ::= SMINUS(B) groupGraphPattern(C). { A = new NTToken(); A->type = 575; A->copyBools(C); A->gGPssVars = C->gGPssVars; A->vars = C->vars; A->bNodes = C->bNodes; A->query = 'MINUS ' . PHP_EOL .  C->query; A->childs = array(B, C); }

groupOrUnionGraphPattern(A) ::= groupGraphPattern(B) groupOrUnionGraphPatternX(C). { A = new NTToken(); A->type = 576; A->copyBools(B); A->copyBools(C); A->gGPssVars = B->gGPssVars + C->gGPssVars; A->vars = B->vars + C->vars; A->bNodes = B->bNodes + C->bNodes; A->query = B->query . PHP_EOL . C->query; A->childs = array(B, C); }
groupOrUnionGraphPattern(A) ::= groupGraphPattern(B). { A = new NTToken(); A->type = 576; A->copyBools(B); A->gGPssVars = B->gGPssVars; A->vars = B->vars; A->bNodes = B->bNodes; A->query = B->query; A->childs = array(B); }
groupOrUnionGraphPatternX(A) ::= groupOrUnionGraphPatternX(B) UNION(C) groupGraphPattern(D). { A = new NTToken(); A->type = 577; A->copyBools(B); A->copyBools(D); A->gGPssVars = B->gGPssVars + D->gGPssVars;  A->vars = B->vars + D->vars; A->bNodes = B->bNodes + D->bNodes; A->query = B->query . PHP_EOL . ' UNION ' . PHP_EOL . D->query; A->childs = array(B, C, D); }
groupOrUnionGraphPatternX(A) ::= UNION(B) groupGraphPattern(C). { A = new NTToken(); A->type = 577; A->copyBools(C); A->gGPssVars = C->gGPssVars;  A->vars = C->vars; A->bNodes = C->bNodes; A->query = 'UNION ' . PHP_EOL . C->query; A->childs = array(B, C); }
...
existsFunc(A) ::= EXISTS(B) groupGraphPattern(C). { A = new NTToken(); A->type = 635; A->copyBools(C); A->ssVars = C->ssVars + C->gGPssVars; A->vars = C->vars; A->bNodes = C->bNodes; A->query = 'EXISTS ' . C->query; A->childs = array(B, C); }

notExistsFunc(A) ::= NOT(B) EXISTS(C) groupGraphPattern(D). { A = new NTToken(); A->type = 636; A->copyBools(D); A->ssVars = D->ssVars + D->gGPssVars; A->vars = D->vars; A->bNodes = D->bNodes; A->query = 'NOT EXISTS ' . D->query; A->childs = array(B, C, D); }
